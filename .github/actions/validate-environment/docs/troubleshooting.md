...
If error occurs, check in this order:

#### 1. Check env: GH_TOKEN Existence

**Check items**:

- [ ] Check if `env: GH_TOKEN: ${{ github.token }}` is set at step or job level
- [ ] Check if indentation (YAML syntax) is correct
- [ ] Verify that `env:` is not a child element of `with:`

**How to check**:

Open workflow file and confirm this structure:

```yaml
- uses: aglabo/.github-aglabo/.github/actions/validate-environment@main
  with:
    additional_apps: "..."
  env: # ← Same level as with
...
- [ ] Check if `${{ github.token }}` is available (normally provided automatically)
- [ ] Verify that "Workflow permissions" is enabled in repository settings

**How to check**:

1. Open GitHub repository **Settings** tab
2. Select **Actions** > **General**
3. Check **Workflow permissions** section
   - Verify that "Read and write permissions" or "Read repository contents and packages permissions" is selected

**Default**: Enabled in most repositories

#### 3. Check Workflow Logs

**Check items**:

- [ ] Open failed workflow in Actions tab
- [ ] Check logs for "Validate Environment" step
- [ ] Check `gh auth status` output

**How to check**:

1. Open GitHub repository **Actions** tab
2. Click failed workflow
3. Expand "Validate Environment" step
...
5. Fix according to error message
...
Instead of local testing, use these methods:

#### 1. Validation via Pull Request
...
#### 3. Pseudo-local Execution with act (Limited)

[act](https://github.com/nektos/act) can simulate GitHub Actions locally:
...
**Note**: act cannot fully reproduce GitHub Actions environment. `RUNNER_ENVIRONMENT=github-hosted` check may fail.
